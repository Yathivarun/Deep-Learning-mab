...
class PiClient:
    ...
    def _receive_loop(self):
        ...
        """
        buffer = b""  # Changed to bytes buffer
        ...
        while self.running:
            try:
                ...
                # This can happen if we're in the middle of receiving binary data
                # Just continue - the message handler will sort it out
                print(f"[PI-CLIENT] Decode warning (expected during binary transfer): {e}")
                continue
            except Exception as e:
    ...
    def _handle_message(self, json_str: str):
        ...
        try:
            ...
            if msg_type == "heartbeat_ack":
                ...
                # Laptop is sending images - we need to receive them NOW
                # This will read binary data directly from socket
                count = msg.get("count", 0)
                print(f"[PI-CLIENT] Receiving {count} images...")
                images = self._receive_images_sync(count)
                
                if self.on_images_received:
                    if images:
                        self.on_images_received(images)
                    else:
                        print("[PI-CLIENT] Failed to receive images")
    ...
    def _receive_images_sync(self, count: int) -> list:
        """
        Receive multiple binary images from laptop.
        This is called synchronously when we receive the "images" message.
        """
        images = []
...
class LaptopServer:
    ...
    def _receive_loop(self):
        ...
        while self.running and self.connected:
            try:
                ...
                # Can happen during mixed JSON/binary data
                continue
            except Exception as e:
    ...
    def send_images(self, image_data_list: list) -> bool:
        ...
        try:
            ...
            # 2. Small delay to ensure message is received before binary data
            time.sleep(0.1)
            
            # 3. Send each image as binary
